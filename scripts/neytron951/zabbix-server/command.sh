...
installZabbix() {
  if [ -e '/etc/redhat-release' ]; then
    ...
    yum clean all
    if [ "$OS_NAME" = "oracle" ] && [ "$(echo "$OS_VERSION" | cut -c1)" = "9" ]; then
      yum install -y pwgen
      yum install -y mariadb-server ${ZABBIX_PKGS} --disablerepo=ol9_developer_EPEL
    elif [ "$OS_NAME" = "almalinux" ] && [ "$(echo "$OS_VERSION" | cut -c1)" = "9" ]; then
      retry_yum_install ${LINUX_PKGS} ${ZABBIX_PKGS}
    else
  ...
  case "${OS_NAME}${OS_VERSION}" in
    debian11)
      ...
      case "${ZABBIX_VERSION}" in
        7.2)
          echo 'Zabbix-server version 7.2 missing for Debian 11'
          return 0
          ;;
        6.4)
    ...
    ubuntu20.04)
      ...
      case "${ZABBIX_VERSION}" in
        7.2)
          echo 'Zabbix-server version 7.2 missing for Ubuntu20.04'
          return 0
          ;;
        6.4)
          ZABBIX_PKGS="${ZABBIX_PKGS} zabbix-sql-scripts"
    ...
    ubuntu24.04)
      ...
      case "${ZABBIX_VERSION}" in
        ...
        6.0)
          ...
          ;;
        5.0)
          echo 'Zabbix-server version 5.0 missing for Ubuntu24.04'
          return 0
          ;;
      esac
      ;;
    almalinux8*)
      ...
      case "${ZABBIX_VERSION}" in
        7.2)
          yum -y module switch-to php:7.4
